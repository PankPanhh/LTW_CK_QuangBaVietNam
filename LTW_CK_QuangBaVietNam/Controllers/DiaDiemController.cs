    ...
    public class DiaDiemController : Controller
    {
        ...
        public ActionResult GetPlaceDetail(string slug)
        {
            try
            {
                ...
                if (place == null)
                {
                    return Json(new { success = false, message = "Không tìm th?y ??a ?i?m" }, JsonRequestBehavior.AllowGet);
                }
        ...
        public ActionResult SearchPlaces(string keyword, int soKetQua = 10)
        {
            try
            {
                ...
                {
                    return Json(new { success = false, message = "Vui lòng nh?p t? khóa" }, JsonRequestBehavior.AllowGet);
                }
        ...
        public ActionResult GetPlacesByRegion(string vungMien, int soKetQua = 9)
        {
            try
            {
                if (string.IsNullOrEmpty(vungMien))
                {
                    return Json(new { success = false, message = "Vui lòng ch? ??nh vùng mi?n" }, JsonRequestBehavior.AllowGet);
                }
